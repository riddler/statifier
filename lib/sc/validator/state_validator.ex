defmodule SC.Validator.StateValidator do
  @moduledoc """
  ...
  """
  @spec validate_state_ids(SC.Validator.validation_result(), SC.Document.t()) ::
          SC.Validator.validation_result()
  def validate_state_ids(%SC.Validator{} = result, %SC.Document{} = document) do
  ...
  """
  @spec validate_unique_ids(SC.Validator.validation_result(), [SC.State.t()]) ::
          SC.Validator.validation_result()
  def validate_unique_ids(%SC.Validator{} = result, states) do
  ...
  """
  @spec validate_non_empty_ids(SC.Validator.validation_result(), [SC.State.t()]) ::
          SC.Validator.validation_result()
  def validate_non_empty_ids(%SC.Validator{} = result, states) do
