defmodule SC.Validator.Utils do
  @moduledoc """
  ...
  """
  @spec add_error(SC.Validator.validation_result(), String.t()) ::
          SC.Validator.validation_result()
  def add_error(%SC.Validator{} = result, error) do
  ...
  """
  @spec add_warning(SC.Validator.validation_result(), String.t()) ::
          SC.Validator.validation_result()
  def add_warning(%SC.Validator{} = result, warning) do
  ...
  def state_exists?(state_id, %SC.Document{} = document) do
  ...
  def collect_all_states(%SC.Document{states: states}) do
  ...
  def get_initial_state(%SC.Document{initial: initial}) when is_binary(initial), do: initial
  def get_initial_state(%SC.Document{states: [first_state | _rest]}), do: first_state.id
  def get_initial_state(%SC.Document{states: []}), do: nil
  ...
  def find_state_by_id_linear(state_id, %SC.Document{} = document) do
    collect_all_states(document)
